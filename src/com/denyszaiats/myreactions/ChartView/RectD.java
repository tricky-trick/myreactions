...
package com.denyszaiats.myreactions.ChartView;
...
/**
 * RectD holds four double coordinates for a rectangle. The rectangle is
...
public class RectD implements Parcelable {
	...
	@Override
	public int hashCode() {
		long result = (left != +0.0f ? Double.doubleToLongBits(left) : 0);
		result = 31 * result + (top != +0.0f ? Double.doubleToLongBits(top) : 0);
		result = 31 * result + (right != +0.0f ? Double.doubleToLongBits(right) : 0);
		result = 31 * result + (bottom != +0.0f ? Double.doubleToLongBits(bottom) : 0);
		return (int) result;
	}
